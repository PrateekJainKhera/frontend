...
function BulkPrintContent() {
  ...
  useEffect(() => {
    ...
    Promise.all(ids.map(id => purchaseRequestService.getById(id)))
      .then(setPRs)
      .catch(() => {})
  ...
  if (prs.length === 0) return <div style={{ padding: 32, textAlign: 'center', fontFamily: 'Arial' }}>No PRs selected</div>
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', fontSize: '12px', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid #e5e7eb', background: '#f9fafb' }} className="print:hidden">
        ...
        >
          🖨 Print All ({prs.length} PR{prs.length > 1 ? 's' : ''})
        </button>
        <button
        ...
        return (
          <div
            ...
            style={{
              ...
              maxWidth: '210mm',
              margin: '0 auto',
              pageBreakAfter: prIndex < prs.length - 1 ? 'always' : 'auto',
            }}
          >
            {/* ── Header ── */}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#ececec' }}>
                  ...
                  <th style={{ border: '1px solid #bbb', padding: '6px 8px', textAlign: 'right', width: 80 }}>Req. Qty</th>
                  {hasApproved && <th style={{ border: '1px solid #bbb', padding: '6px 8px', textAlign: 'right', width: 80 }}>Appr. Qty</th>}
                  <th style={{ border: '1px solid #bbb', padding: '6px 8px', textAlign: 'left', width: 100 }}>Vendor</th>
                  <th style={{ border: '1px solid #bbb', padding: '6px 8px', textAlign: 'center', width: 75 }}>Status</th>
                </tr>
              </thead>
              <tbody>
            ...
            <div style={{ marginTop: 24, paddingTop: 10, borderTop: '1px solid #ddd', textAlign: 'center', fontSize: 10, color: '#999' }}>
              ...
              {prs.length > 1 && <span> · {prIndex + 1} of {prs.length}</span>}
            </div>
          </div>
        )
      })}
